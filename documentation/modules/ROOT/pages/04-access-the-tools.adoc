...
== 4. Set up your GitOps repository

Following the GitOps practices, *all the deployment configuration is stored in the git repo*. We have created an initial repository that you will change to configure your deployment. As part of the demo, you will have to do some changes and commits. Fork the repository, as you don't have write permissions on the initial git repo.

=== 4.1. Fork the initial repository

Access the https://github.com/davidseve/cloud-native-deployment-strategies[cloud-native-deployment-strategies] repository and fork it on your GitHub account. This will be your `gitops` repository for your application. The deployment configuration in the repository will be synced to Openshift.
...
image::github-repo-fork-01.png["Click to fork the repository"]

On the forking page, leave the config as shown in the screenshot and click `Fork`:

.GitHub fork repository process
image::github-repo-fork-02.png["GitHub fork repository process"]

If prompted, choose to fork the `main` branch.

.How to know if it worked?
[TIP]
====
The original link to the repository was `https://github.com/davidseve/cloud-native-deployment-strategies`, but after forking, you should be in a different URL and it should contain your GitHub name. This means that the repository is yours and that you have `write` permissions. For example, the user `alvarlop` will have its repository here:

    https://github.com/alvarolop/cloud-native-deployment-strategies
====

=== 4.2. Configure your repository

Now, in your forked repository, go to `Settings > General > Pull Requests` and configure the repo to `Automatically delete head branches` after a PR is merged. This is a best practice in Git. We also need it for our pipelines if you want to run the same step several times.

.Configure the repo to autodelete branches
image::github_pr_delete_branch.png["Configure the repo to autodelete branches"]
...
[TIP]
====
In this section, you will generate a GitHub token for two tasks:

* The automated pipelines will use it to authenticate to GitHub and modify the git repository.
* You will use it if you clone the repository using the HTTPS method instead of SSH.
====
...
* Expiration: `7 days`.
* Description: `Token to manage my cloud-native-deployment-strategies repository from automation pipelines`.
...
== 6. Clone and add a new branch

Finally, clone the repository, generate a new local branch named `release` and push it to GitHub using the following commands: 
...
When you execute the fourth command, you will be prompted for a username and password. The username will be the same as %REPOSITORY%, while the password will be the PAT that you generated previously.

.How to know if it worked?
[TIP]
====
This is probably the *most important step of this chapter*. So, make sure that it worked!!

The last command has to create a new branch in the GitHub repository. The output should be similar to the one that follows:

[source, console]
----
git push origin release
Username for 'https://github.com': alvarolop
Password for 'https://alvarolop@github.com': 
Total 0 (delta 0), reused 0 (delta 0), pack-reused 0
remote: 
remote: Create a pull request for 'release' on GitHub by visiting:
remote:      https://github.com/alvarolop/cloud-native-deployment-strategies/pull/new/release
remote: 
To https://github.com/alvarolop/cloud-native-deployment-strategies
 * [new branch]      release -> release
----

You can also check that the branch is created in the web console. Go to the `branches` section of your repository (https://github.com/%REPOSITORY%/cloud-native-deployment-strategies/branches) and check that there are two branches: `main` and `release`.
====
