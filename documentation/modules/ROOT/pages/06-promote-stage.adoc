...
We will start deploying a new version `v1.1.1` in the offline color. And get it ready to received online users traffic
...
We can get the offline color form Openshift service or from the file `values/products/envs/stage/non-static-networking-values.yaml`. Then edit the following files in the right environment and color.
...
Edit the file `values/products/envs/stage/green/image-tag-values.yaml`.
...
Edit the file `values/products/envs/stage/green/non-static-values.yaml`.
...
See that offline color have version `v1.1.1` and the new attribute description, but the online has not changed.
...
[source,input,subs="+macros,+attributes"]
----
oc create -f 3-pipelinerun-products-aling-offline.yaml -n {user}-continuous-deployment
----
--
====
